...
def upgrade():
    ...
    if dialect=="postgresql": op.execute("CREATE EXTENSION IF NOT EXISTS vector")
    ...
    tables=sa.inspect(bind).get_table_names(); embedding_type=Vector(1024) if dialect=="postgresql" else sa.JSON()
    ...
    if dialect=="postgresql" and "ix_document_chunks_embedding_hnsw" not in chunk_indexes: op.execute("CREATE INDEX ix_document_chunks_embedding_hnsw ON document_chunks USING hnsw (embedding vector_cosine_ops)")
