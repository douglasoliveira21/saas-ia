...
def create_token(user, minutes=30): return jwt.encode({"sub":user.id,"company_id":user.company_id,"role":user.role,"exp":datetime.now(timezone.utc)+timedelta(minutes=minutes)},settings.secret_key,algorithm=ALGORITHM)
...
def current_user(credentials:HTTPAuthorizationCredentials=Depends(bearer),db:Session=Depends(get_db)):
    ...
    try: user_id=jwt.decode(credentials.credentials,settings.secret_key,algorithms=[ALGORITHM])["sub"]
